// Center column: shows ONE of three views — the product, a chair caster, or a room scene.
function ProductHero({ v, view = "product" }) {
  return (
    <div style={{ position: "relative", height: "min(660px, 78vh)", display: "grid", placeItems: "center" }}>
      {/* Arched backdrop */}
      <svg viewBox="0 0 480 660" style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }} preserveAspectRatio="xMidYMid meet">
        <defs>
        ...
        <path d="M 80 620 L 80 260 A 160 160 0 0 1 400 260 L 400 620 Z" fill={`url(#arch-${v.key})`}/>
      </svg>

      {/* Orbital ring */}
      {view === "product" && (
        <svg viewBox="0 0 480 660" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", animation: "ogSpin 28s linear infinite", opacity: 0.45 }} preserveAspectRatio="xMidYMid meet">
          <ellipse cx="240" cy="380" rx="220" ry="58" fill="none" stroke={v.dark ? "rgba(255,255,255,0.22)" : "rgba(0,0,0,0.10)"} strokeWidth="1" strokeDasharray="2 4"/>
        </svg>
      )}
...
function PuckView({ v }) {
  return (
    <div style={{
      ...
      <div style={{
        position: "absolute", inset: "50%", width: 92, height: 92,
        transform: "translate(-50%,-50%)", borderRadius: "50%",
        background: v.bg, boxShadow: "inset 0 0 0 6px rgba(0,0,0,0.06)",
      }}/>
    </div>
  );
}

// A side-view office chair with all five casters wrapped in the variant color.
function ChairView({ v }) {
  const stroke = v.dark ? "rgba(255,255,255,0.95)" : "#1A1B1F";
  const soft   = v.dark ? "rgba(255,255,255,0.06)" : "#FFFFFF";
  const sw = 4;
  // five wheel positions along the floor
  const wheels = [
    { cx: 70,  cy: 320 },
    { cx: 130, cy: 332 },
    { cx: 195, cy: 338 },
  ...
  return (
    <svg viewBox="0 0 400 400" width="380" height="380">
      {/* Backrest */}
      <path d="M 130 60 Q 130 30 160 30 L 230 30 Q 260 30 260 60 L 260 175 Q 260 200 235 200 L 155 200 Q 130 200 130 175 Z"
            fill={soft} stroke={stroke} strokeWidth={sw} strokeLinejoin="round"/>
      {/* Cushion seam on back */}
      <line x1="195" y1="50" x2="195" y2="180" stroke={stroke} strokeWidth="1.5" strokeDasharray="3 6" opacity="0.5"/>

      {/* Arms */}
      <path d="M 120 175 L 90 175 Q 80 175 80 185 L 80 215 Q 80 225 90 225 L 120 225"
            fill="none" stroke={stroke} strokeWidth={sw} strokeLinecap="round" strokeLinejoin="round"/>
      <path d="M 270 175 L 300 175 Q 310 175 310 185 L 310 215 Q 310 225 300 225 L 270 225"
            fill="none" stroke={stroke} strokeWidth={sw} strokeLinecap="round" strokeLinejoin="round"/>

      {/* Seat */}
      <path d="M 95 200 L 295 200 Q 310 200 308 215 L 300 240 Q 298 250 285 250 L 105 250 Q 92 250 90 240 L 82 215 Q 80 200 95 200 Z"
      ...
      {/* Five-arm base */}
      {wheels.map((w, i) => (
        <line key={`leg-${i}`} x1="195" y1="300" x2={w.cx} y2={w.cy - 8} stroke={stroke} strokeWidth={sw} strokeLinecap="round"/>
      ))}
      {/* Hub at base */}
      ...
      {/* Floor highlight under the chair */}
      <ellipse cx="200" cy="362" rx="160" ry="4" fill={v.dark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.08)"}/>

      {/* Callout: arrow + label */}
      <line x1="320" y1="308" x2="360" y2="270" stroke={stroke} strokeWidth="1.5"/>
      <circle cx="362" cy="268" r="3" fill={stroke}/>
      <text x="310" y="260" fontFamily="var(--font-mono)" fontSize="11" fill={stroke} textAnchor="end">All 5 casters wrapped</text>
    </svg>
  );
}

// Cozy room scene with a desk, chair, pet, and a plant.
function RoomView({ v }) {
  ...
  return (
    <svg viewBox="0 0 420 380" width="420" height="380">
      ...
      {/* Chair (side view, simplified) */}
      <path d="M 215 160 Q 215 130 230 130 L 252 130 Q 252 200 252 220" fill="none" stroke={stroke} strokeWidth={sw} strokeLinecap="round" strokeLinejoin="round"/>
      <path d="M 215 215 L 270 215 Q 280 215 280 225 L 280 235 Q 280 245 270 245 L 215 245 Q 205 245 205 235 L 205 225 Q 205 215 215 215 Z" fill={v.hex} stroke={stroke} strokeWidth={sw}/>
      ...
      {/* Tail curling toward the chair */}
      <path d="M 162 310 Q 180 305 195 295" fill="none" stroke={stroke} strokeWidth={sw} strokeLinecap="round"/>

      {/* Caption */}
      <text x="210" y="365" fontFamily="var(--font-mono)" fontSize="11" fill={stroke} textAnchor="middle" opacity="0.7">Tails, cables and toes — all safe.</text>
    </svg>
  );
}

function FeatureChip({ v, chip, style }) {
  const dark = v.dark;
  return (
    <div style={{
      ...style,
      ...
      <span style={{
        ...
        background: v.key === "polar" ? v.ring : v.hex, color: "white",
        display: "grid", placeItems: "center", flexShrink: 0,
      }}>
        <i data-lucide={chip.icon} style={{ width: 14, height: 14, strokeWidth: 2 }}></i>
      </span>
      <span>{chip.text}</span>
    </div>
  );
...
Object.assign(window, { ProductHero, FeatureChip });
