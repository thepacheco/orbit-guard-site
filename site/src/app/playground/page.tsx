...
export default function PlaygroundPage() {
  ...
  const [camPosStr, setCamPosStr] = useState('[97.41, -66.94, 171.29]');
  const [camPosArray, setCamPosArray] = useState<[number, number, number]>([97.41, -66.94, 171.29]);
  ...
  const resetStudio = () => {
    ...
    setExploded(false);
    setFloatMode(false);
    applyPreset([97.41, -66.94, 171.29], false, false);
  };
      ...
      <div style={{ maxWidth: 1380, margin: '0 auto', padding: '0 40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 40, alignItems: 'start' }}>
          ...
          <div>
            ...
            {/* Live Camera Vector Coordinates Display Card — Dark Mode */}
            <div
              style={{
                marginTop: 16,
                padding: '14px 20px',
            ...
            <div style={{ marginTop: 20, display: 'flex', gap: 14, alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => applyPreset([97.41, -66.94, 171.29], false, false)} style={{ padding: '12px 16px', background: '#fff', border: '1px solid #e5e7eb', color: '#374151', borderRadius: 10, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
                  3D View
                </button>
                <button
                  onClick={() => {
